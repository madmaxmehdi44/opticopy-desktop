...
public static class OpticalFileContainer
{
    ...
    public static async Task<PackedFile> PackAsync(string fileName, string mimeType, ReadOnlyMemory<byte> bytes, CancellationToken cancellationToken = default)
    {
        ...
        byte[]? compressed = tryGzip ? await GzipAsync(original, cancellationToken).ConfigureAwait(false) : null;
    ...
    public static async Task<UnpackedFile> UnpackAsync(ReadOnlyMemory<byte> container, CancellationToken cancellationToken = default)
    {
        ...
        {
            if (transmitted.Length < 18) throw new InvalidDataException("The gzip payload is incomplete.");
            if (ReadUInt32LE(transmitted, transmitted.Length - 4) != fileLength)
    ...
    private static async Task<byte[]> GunzipAsync(byte[] bytes, int maxBytes, CancellationToken cancellationToken)
    {
        ...
        await using var gzip = new GZipStream(source, System.IO.Compression.CompressionMode.Decompress, leaveOpen: false);
        await using var destination = new MemoryStream(Math.Min(maxBytes, Math.Max(256, bytes.Length * 2)));
        ...
        while (true)
        {
            ...
            if (read == 0) break;
            total += read;
            if (total > maxBytes) throw new InvalidDataException("The gzip payload expands beyond the DCF2 file limit.");
    ...
    private static bool IsPrecompressedType(string type)
    {
        ...
        if (media.StartsWith("image/", StringComparison.Ordinal))
            return media is not ("image/bmp" or "image/x-ms-bmp" or "image/svg+xml" or "image/tiff" or "image/x-icon" or "image/vnd.microsoft.icon");
        if (media.StartsWith("audio/", StringComparison.Ordinal))
            return media is not ("audio/wav" or "audio/x-wav" or "audio/wave" or "audio/vnd.wave" or "audio/aiff" or "audio/x-aiff" or "audio/basic" or "audio/l16");
        if (media.StartsWith("application/vnd.openxmlformats-officedocument.", StringComparison.Ordinal) || media.StartsWith("application/vnd.oasis.opendocument.", StringComparison.Ordinal) || media.EndsWith("+zip", StringComparison.Ordinal)) return true;
        return media is "application/gzip" or "application/java-archive" or "application/vnd.rar" or "application/x-7z-compressed" or "application/x-brotli" or "application/x-bzip" or "application/x-bzip2" or "application/x-gzip" or "application/x-lzma" or "application/x-rar-compressed" or "application/x-xz" or "application/x-zip-compressed" or "application/zip" or "application/zstd";
    }
