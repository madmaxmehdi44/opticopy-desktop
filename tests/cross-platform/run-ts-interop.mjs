...
const frame = protocol.packFrame({
  sessionId: 0xBEEF, seq: 0x01020304, k: 3, blockLen: framePayload.length,
  totalLen: packed.container.length, payloadFnv: protocol.fnv1a(packed.container), flags: 0,
}, framePayload);

const encoder = new fountain.LTEncoder(packed.container, 8, 0xBEEF);
const fountainFrames = [0, 1, 2, 3, 4, 5].map((seq) => Buffer.from(protocol.packFrame({
  sessionId: 0xBEEF, seq, k: encoder.k, blockLen: 8, totalLen: packed.container.length,
  payloadFnv: protocol.fnv1a(packed.container), flags: 0,
}, encoder.encode(seq))).toString("base64"));

writeFileSync(join(fixtureRoot, "ts-to-cs.json"), JSON.stringify({
  source: Buffer.from(source).toString("base64"),
  dcf2: Buffer.from(packed.container).toString("base64"),
  frame: Buffer.from(frame).toString("base64"),
  fountain: { blockLength: 8, sessionId: 0xBEEF, totalLength: packed.container.length,
    k: encoder.k, payloadFnv: protocol.fnv1a(packed.container), frames: fountainFrames },
}, null, 2));

const csPath = join(fixtureRoot, "cs-to-ts.json");
const cs = JSON.parse(readFileSync(csPath, "utf8"));
const unpacked = await protocol.unpackFile(Buffer.from(cs.dcf2, "base64"));
if (Buffer.from(unpacked.bytes).toString("base64") !== cs.source) throw new Error("TypeScript failed to recover C# DCF2 source bytes");
...
if (Buffer.from(recoveredFile.bytes).toString("base64") !== cs.source) throw new Error("TypeScript fountain recovery bytes differ from source");
